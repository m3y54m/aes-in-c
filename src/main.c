...
// Pads 'data' in-place up to 'block_size'. 'len' is the length of input (before padding), up to block_size.
// Returns the new padded length (always block_size), or 0 on error.
static inline size_t pkcs7_pad(unsigned char *data, size_t len, size_t block_size) {
    if (block_size == 0 || block_size > 255 || len > block_size) return 0;
    unsigned char pad = (unsigned char)(block_size - len);
    for (size_t i = len; i < block_size; ++i)
        data[i] = pad;
    return block_size;
}

// Removes PKCS#7 padding in-place from 'data' of length 'len' (should be a multiple of block_size).
// Returns the new length (unpadded), or 0 if padding is invalid.
static inline size_t pkcs7_unpad(unsigned char *data, size_t len, size_t block_size) {
    if (len == 0 || block_size == 0 || len % block_size != 0) return 0;
...
int main(int argc, char *argv[])
{
    ...
    unsigned char plaintext[] = "now you can process data not multiple of block size!";
    ...
    size_t pad_len = block_size - (pt_len % block_size);
    size_t padded_len = pt_len + pad_len;

    // Allocate buffers
    unsigned char *padded_plain = (unsigned char *)malloc(padded_len);
    unsigned char *ciphertext = (unsigned char *)malloc(padded_len);
    unsigned char *decrypted = (unsigned char *)malloc(padded_len);

    // Copy and pad
    memcpy(padded_plain, plaintext, pt_len);
    pkcs7_pad(padded_plain + (padded_len - block_size), pt_len % block_size, block_size);

    // Encrypt each block
    for (size_t i = 0; i < padded_len; i += block_size)
        aes_encrypt(padded_plain + i, ciphertext + i, key, size);

    // Print ciphertext
    printf("Ciphertext (HEX):\n");
    for (size_t i = 0; i < padded_len; i++)
        printf("%02x%s", ciphertext[i], ((i+1)%block_size) ? " " : "\n");

    // Decrypt each block
    for (size_t i = 0; i < padded_len; i += block_size)
        aes_decrypt(ciphertext + i, decrypted + i, key, size);

    // Unpad
    size_t unpadded_len = padded_len;
    if (padded_len >= block_size)
        unpadded_len = ((padded_len - block_size) + pkcs7_unpad(decrypted + padded_len - block_size, block_size, block_size));

    // Print decrypted text
    printf("\nDecrypted text:\n");
    fwrite(decrypted, 1, unpadded_len, stdout);
    printf("\n");
